...
| `resume-mode` | What automation answers when a relaunch meets Claude's resume dialog (long conversations): `as-is` (default) keeps the FULL context and never summarizes without being asked (huge conversations may reload slowly); `summary` resumes from a compacted summary. |
...
| `notify-command` | Optional. A command run as `<command> "<message>"` to alert YOU when something needs a human: a session logged out of Claude, a managed session that can't be healed, a failed bus request (throttled to once per 4h per condition). `agent-nexus setup-telegram` walks the whole Telegram flow, from bot creation to a test message. Empty = off. Named channels: extra config lines `notify-channel-<name>: <command>` define alternate routes (a second Telegram bot is one line, pointing notify-telegram.sh at a different env file via `TELEGRAM_ENV_FILE`); a task opt `notify=<name>` sends that task's failure alerts there, and `agent-nexus notify [--channel <name>] "<message>"` lets a task's own agent report through the same routing (`notify --list` shows the routes). |
...
| `context-watch` | `on` (default): every scheduler tick reads each Active session's context occupancy straight from its own transcript file (free; nothing is ever typed into a session) and surfaces it: `ctx:NN%` badges in the hub, a live context row on the session info screen, a status-panel warning past the thresholds, a line in `/status` on the control bot, and a per-session history log (readings plus inferred compaction/clear drops) in the state dir's `context-watch/`. `context-notice` (default 45) is the yellow threshold, `context-act` (default 60) the "refresh the handoff, then compact or clear" one; crossing act is action-logged, and `context-telegram: on` also sends a phone FYI. `context-window` (`auto` default, or `1m` / `200k`): the transcript cannot reveal the window size, so `auto` assumes 200k and promotes a conversation to 1M (stickily) once its usage proves it; if you KNOW your plan runs 1M windows, set `1m` and the percentages are right immediately. Tier 2 (`context-watch install-hook <session>`) adds a hook that shows the model its own percent each turn; tier 3 steers checkpoint-compact managed sessions into docs-first self-compaction past the act threshold, throttled, idle-only, with `context-watch pause|resume|off` as the human override. A session can checkpoint by CLEARING instead (`checkpoint-compact: clear` in managed-sessions.md) when a carried summary is dead weight; the resume re-captures the new conversation id. `context-report` (`all` default, `off`, or a comma list of `heal,restore,boot,digest`) logs the percent after lifecycle events and adds a per-session Context section to the daily digest. Smart reset: a managed session's pre-run `reset: compact` is skipped when the gauge reads below the act threshold. |
...
| `/compact <name>` | Ask that session to wrap up docs-first and compact (human override of the Context Watch gates; the steer names the day's handoff) |
| `/fire <task-id>` | Fire a scheduled or event task now, as caller `telegram` (task ids are validated; a busy target queues the fire; results come back honestly, including a script task's own failure) |
| `/help` | The list above |
...
- **checkpoint-compact**: `none` (default), `compact`, or `clear` - exactly
  the same vocabulary as `reset` - lets a long-running session shed its own
  context to cut token cost. With `compact`, the session compacts at
  boundaries *it* declares: after committing a unit and updating its docs, it
  runs `agent-nexus compact-checkpoint --next "<next step>"` and ends its
  turn; the tool injects `/compact` (Claude Code can't self-trigger it),
  waits for it to settle, confirms the session is responsive, then re-prompts
  it to continue. With `clear`, the same protocol checkpoints by `/clear`
  instead (a brand-new conversation, its id re-captured into the registry):
  the docs the session just wrote ARE the memory, for stateless recurring
  work where a carried summary is dead weight. (Legacy spellings are
  accepted and migrated: `on` means `compact`, `off` means `none`.) Set it
