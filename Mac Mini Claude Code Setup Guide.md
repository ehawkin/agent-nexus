...
| `context-watch` | `on` (default): every scheduler tick reads each Active session's context occupancy straight from its own transcript file (free; nothing is ever typed into a session) and surfaces it: `ctx:NN%` badges in the hub, a live context row on the session info screen, a status-panel warning past the thresholds, a line in `/status` on the control bot, and a per-session history log (readings plus inferred compaction/clear drops) in the state dir's `context-watch/`. `context-notice` (default 45) is the yellow threshold, `context-act` (default 60) the "refresh the handoff, then compact or clear" one; crossing act is action-logged, and `context-telegram: on` also sends a phone FYI. `context-window` (`auto` default, or `1m` / `200k`): the transcript cannot reveal the window size, so `auto` assumes 200k and promotes a conversation to 1M (stickily) once its usage proves it; if you KNOW your plan runs 1M windows, set `1m` and the percentages are right immediately. Tier 2 (`context-watch install-hook <session>`) adds a hook that shows the model its own percent each turn; tier 3 steers checkpoint-compact managed sessions into docs-first self-compaction past the act threshold, throttled, idle-only, with `context-watch pause|resume|off` as the human override. |
